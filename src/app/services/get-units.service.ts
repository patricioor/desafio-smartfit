...
export class GetUnitsService {
  ...
  private allUnisSubject: BehaviorSubject<Location[]> = new BehaviorSubject<Location[]>([])
  private allUnits$: Observable<Location[]> = this.allUnisSubject.asObservable();
  private filteredUnits: Location[] = []

  constructor(private httpClient: HttpClient) {
    this.httpClient.get<UnitsResponse>(this.apiUrl).subscribe(data => {
      this.allUnisSubject.next(data.locations);
