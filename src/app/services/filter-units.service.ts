...
export class FilterUnitsService {
    ...
    for( let i = 0; i < unit.schedules.length; i++) {
      ...
      if(today_weekday === schedule_weekday){
        if(schedule_hour !== 'Fechada'){
          ...
          let unit_open_hour_filter = parseInt(unit_open_hour.replace('h', ''), 10)
          let unit_close_hour_filter = parseInt(unit_close_hour.replace('h', ''), 10)

          return open_hour_filter <= unit_open_hour_filter && close_hour_filter <= unit_close_hour_filter;
        }
      }
    }
    return false;
  ...
  filter(results: Location[], showClosed: boolean, hour: string){
      ...
      if(!showClosed) {
        let intermediateResults = results.filter(location => location.opened)
      }
